...
def submit_order(stock_data: dict, scan_context: dict = None) -> bool:
    ...
    # Cap total pending orders
    with _state_lock:
        total_pending = sum(len(v) for v in _pending_orders.values())
        if total_pending >= MAX_PENDING_ORDERS:
            log.warning("[EXEC] Order rejected: MAX_PENDING_ORDERS (%d) reached", MAX_PENDING_ORDERS)
            with _stats_lock:
                _stats["orders_rejected_capacity"] += 1
            return False

    # Duplicate prevention: check cooldown + existing positions/orders
    if not _check_cooldown(sym):
        return False
...
def _check_cooldown(symbol: str) -> bool:
    ...
    with _state_lock:
        if symbol in _active_positions and _active_positions[symbol]:
            log.debug("[EXEC] Cooldown: %s already has open position", symbol)
            with _stats_lock:
                _stats["orders_rejected_duplicate"] += 1
            return False
        if symbol in _pending_orders and _pending_orders[symbol]:
            log.debug("[EXEC] Cooldown: %s already has pending order", symbol)
            with _stats_lock:
                _stats["orders_rejected_duplicate"] += 1
            return False

    # Check DB cooldown (5-day window)
    try:
        import db
