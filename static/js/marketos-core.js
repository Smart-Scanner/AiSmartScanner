/**
 * ═══════════════════════════════════════════════════════════════════════
 * MarketOS Core JS — Frozen Contract Layer
 * ═══════════════════════════════════════════════════════════════════════
 * 
...
const MarketOS = (function () {
    ...
    function createEvent(eventType, producerDomain, params) {
        ...
        if (!expectedProducer) {
            console.error(`[MarketOS] Unknown event type: ${eventType}`);
            return null;
        }
        if (expectedProducer !== producerDomain) {
            console.error(`[MarketOS] Domain '${producerDomain}' cannot produce '${eventType}'. Owner: '${expectedProducer}'`);
            return null;
        }

        const entityType = params.entity_type || 'Asset';
        if (!VALID_ENTITY_TYPES.includes(entityType)) {
            console.warn(`[MarketOS] Unknown entity_type: ${entityType}`);
        }

        const source = params.source || 'manual';
        if (!VALID_SOURCES.includes(source)) {
            console.warn(`[MarketOS] Unknown source: ${source}`);
        }
        ...
        if (!validation.valid) {
            validation.errors.forEach(err => console.error(`[MarketOS] ${err}`));
    ...
    function _dispatch(event) {
        ...
        listeners.forEach(cb => {
            try { cb(event); } catch (e) { console.error('[MarketOS] Event handler error:', e); }
        });
    }
    ...
    function _storageSet(key, value) {
        try { localStorage.setItem(key, JSON.stringify(value)); } catch (e) { console.error('[MarketOS] Storage write failed:', e); }
    }
    ...
    const JournalRepository = {
        ...
        addEntry(entry) {
            ...
            if (!symbol) { console.error('[MarketOS] Journal entry requires symbol'); return null; }
    ...
    const ScanRepository = {
        ...
        saveScanResult(params) {
            if (!params.scan_type) {
                console.error('[MarketOS] ScanResult requires scan_type');
                return null;
            }
    ...
    const MarketDataRepository = {
        ...
        upsert(params) {
            if (!params.symbol || !params.exchange) {
                console.error('[MarketOS] SymbolMaster requires symbol and exchange');
                return null;
            }
    ...
    const IntelligenceRepository = {
        ...
        saveProfile(params) {
            if (!params.instrument_id) {
                console.error('[MarketOS] IntelligenceProfile requires instrument_id');
                return null;
            }
    ...
    function createResearchSnapshot(params) {
        ...
        if (sourceCandidateId && !params.parent_snapshot_id) {
            ...
            if (existingFromCandidate) {
                console.error(`[MarketOS] IDEMPOTENCY: Snapshot already exists for candidate '${sourceCandidateId}' (snapshot: ${existingFromCandidate.snapshot_id}). Use parent_snapshot_id for revisions.`);
    ...
    function createPosition(params) {
        console.warn('[MarketOS] DEPRECATED: Direct Position creation is deprecated in 5.0. Use Broker Hub OMS flow (PLACE_ORDER → BrokerExecution).');
    ...
    function createPortfolio(params) {
        if (!params.name) {
            console.error('[MarketOS] Portfolio requires name');
            return null;
        }
    ...
    function migratePositionSectors() {
        ...
        if (migrated > 0) {
            console.log(`[MarketOS] Legacy migration: patched ${migrated}/${total} positions with sector_id`);
        }
    ...
    const PortfolioRepository = {
        ...
        updatePosition(portfolioId, positionId, updates) {
            ...
            if (idx < 0) {
                console.error(`[MarketOS] Position ${positionId} not found in portfolio ${portfolioId}`);
                return null;
            }
    ...
    async function getPortfolios() {
        ...
        try {
            ...
            console.warn('[MarketOS] Portfolio API unavailable, using local portfolios');
        }
    ...
    function _renderNotifBody(body) {
        ...
        body.innerHTML = _notifications.slice(0, 20).map(n => {
            ...
            return `<div class="mos-notif-item ${n.read ? '' : 'unread'}" onclick="MarketOS.markNotificationRead('${n.id}')">
    ...
    function _getCurrentUserId() {
        ...
        return window.__MOS_USER_ID || 'anonymous';
    }
    ...
    const ReconciliationEngine = (function () {
        ...
        /**
         * Adopt an UnmanagedPosition into MarketOS.
    ...
    function createActor(params = {}) {
        ...
        if (!ACTOR_TYPES.includes(actorType)) {
            console.error(`[MarketOS:CommandBus] Invalid actor_type: '${actorType}'`);
            return null;
        }
        return Object.freeze({
    ...
    const _COMMAND_HANDLERS = {
        ...
        'CONSUME_INTENT': function(params) {
            ...
            console.warn('[MarketOS] CONSUME_INTENT is deprecated. Use PLACE_ORDER or BrokerHub processIntent.');
        ...
        'CREATE_POSITION': function(params) {
            console.warn('[MarketOS] CREATE_POSITION command is deprecated. Use PLACE_ORDER.');
        ...
        'PLACE_ORDER': async function(params) {
            ...
            console.warn('[MarketOS] PLACE_ORDER is deprecated in 5.2.2. Use CREATE_BROKER_ORDER.');
    ...
    const ExecutionContext = (function() {
        ...
        function set(contextType) {
            if (!VALID_CONTEXTS.includes(contextType)) {
                console.error(`[MarketOS:ExecutionContext] Invalid context: '${contextType}'`);
                return false;
            }
