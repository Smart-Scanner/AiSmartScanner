# Deploying Smart Screener on Ubuntu
...
```bash
scp /tmp/smart-screener-deploy.zip root@YOUR_SERVER_IP:/tmp/
```

Back on the server:
```bash
mkdir -p /opt/smart-screener
unzip /tmp/smart-screener-deploy.zip -d /opt/smart-screener
rm /tmp/smart-screener-deploy.zip    # remove the zip — it contains secrets
chown -R smart:smart /opt/smart-screener
```
...
Lock the file down:
```bash
chmod 600 /opt/smart-screener/.env
```
...
```bash
cat > /etc/systemd/system/smart-screener.service <<'EOF'
[Unit]
Description=Smart Screener (Flask + Gunicorn)
...
User=smart
Group=smart
WorkingDirectory=/opt/smart-screener
EnvironmentFile=/opt/smart-screener/.env
ExecStart=/opt/smart-screener/venv/bin/gunicorn \
    --workers 1 \
    --threads 4 \
    --bind 127.0.0.1:5050 \
    --timeout 120 \
    --access-logfile /var/log/smart-screener.access.log \
    --error-logfile /var/log/smart-screener.error.log \
    app:app
Restart=always
RestartSec=5
StandardOutput=append:/var/log/smart-screener.stdout.log
StandardError=append:/var/log/smart-screener.stderr.log
...
touch /var/log/smart-screener.{access,error,stdout,stderr}.log
chown smart:smart /var/log/smart-screener.*.log

systemctl daemon-reload
systemctl enable --now smart-screener
systemctl status smart-screener --no-pager
```
...
```bash
cat > /etc/nginx/sites-available/smart-screener <<'EOF'
server {
    listen 80;
    listen [::]:80;
...
sed -i 's/app\.yourdomain\.com/REAL_DOMAIN_HERE/g' /etc/nginx/sites-available/smart-screener

# Activate
ln -sf /etc/nginx/sites-available/smart-screener /etc/nginx/sites-enabled/
...
```bash
# Tail live logs
tail -f /var/log/smart-screener.stdout.log
tail -f /var/log/smart-screener.error.log

# Restart after .env change
systemctl restart smart-screener

# Check status / failure cause
systemctl status smart-screener
journalctl -u smart-screener -n 100 --no-pager

# Add another admin
sudo -iu smart bash -c "cd /opt/smart-screener && source venv/bin/activate && python scripts/add_admin.py new-admin@example.com"

# Backup the DB (do this on a cron — auth.db has all users/payments/devices)
sqlite3 /opt/smart-screener/cache/auth.db ".backup '/var/backups/auth-$(date +%Y%m%d).db'"
```
...
# On the server:
systemctl stop smart-screener
unzip -o /tmp/smart-screener-deploy.zip -d /opt/smart-screener
chown -R smart:smart /opt/smart-screener
sudo -iu smart bash -c "cd /opt/smart-screener && source venv/bin/activate && pip install -r requirements.txt"
systemctl start smart-screener
rm /tmp/smart-screener-deploy.zip
```
...
| Live prices not updating | Angel One credentials wrong or WebSocket connection failing. Check `journalctl -u smart-screener` for `Angel One login failed`. |
...
| What | Where |
|---|---|
| App dir | `/opt/smart-screener/` |
| Virtualenv | `/opt/smart-screener/venv/` |
| `.env` | `/opt/smart-screener/.env` (chmod 600) |
| DBs | `/opt/smart-screener/cache/auth.db`, `screener.db` |
| Uploads | `/opt/smart-screener/cache/uploads/qr/`, `cache/uploads/payments/` |
| Logs | `/var/log/smart-screener.{access,error,stdout,stderr}.log` |
| systemd unit | `/etc/systemd/system/smart-screener.service` |
| Nginx vhost | `/etc/nginx/sites-available/smart-screener` |
