"""
Multi-Timeframe Trend Alignment Engine — Phase 2 (Memory Cache + yf_guard)
---------------------------------------------------------------------------
FIXED:
...
def get_mtf_trend(symbol: str, cache_only: bool = False) -> tuple:
    ...
    Args:
        ...
        cache_only: If True, return ({}, 0) on cache miss instead of calling yfinance.
                    Default True behaviour is set by callers (Fast Scan uses True).
    """
    ...
    if not yf_is_available():
        log.debug("MTF yf_guard OPEN for %s — returning empty", sym)
        return {}, 0
...
def prefetch_mtf_batch(symbols: list, max_workers: int = 5) -> None:
    ...
    Skips symbols already cached; respects yf_guard circuit.
    Called during pre-scan warmup for FNO / shortlisted stocks.
    """
    if not yf_is_available():
        log.warning("MTF prefetch skipped: yf_guard circuit OPEN")
        return
